...
var General = GObject.registerClass(
    class MyWeatherGeneralPage extends Adw.PreferencesPage {
        _init(settings) {
            super._init({
                title: _("Settings"),
            ...
            let currentRefreshSpinButton = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({
                    ...
                    step_increment: 1,
                    page_increment: 10,
                    value: this.settings.get_int('refresh-interval-current')  / 60
                }),
                climb_rate: 5,
                numeric: true,
            ...
            let forecastRefreshSpinButton = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({
                    ...
                    step_increment: 1,
                    page_increment: 10,
                    value: this.settings.get_int('refresh-interval-forecast')  / 60
                }),
                climb_rate: 5,
                numeric: true,
                update_policy: 'if-valid',
            ...
            let systemIconsSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('use-system-icons')
            });
            ...
            let startupDelaySpinButton = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({
                    ...
                    step_increment: 1,
                    page_increment: 10,
                    value: this.settings.get_int('delay-ext-init')
                }),
                climb_rate: 1,
                numeric: true,
                update_policy: 'if-valid',
            ...
            let temperatureUnitRow = new Adw.ComboRow({
                ...
                selected: this.settings.get_enum('unit')
            });
            ...
            let windSpeedUnitRow = new Adw.ComboRow({
                ...
                selected: this.settings.get_enum('wind-speed-unit')
            });
            ...
            let pressureUnitRow = new Adw.ComboRow({
                title: _("Pressure"),
                model: pressureUnits,
                selected: this.settings.get_enum('pressure-unit')
            });
            ...
            let providerTranslateSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('owm-api-translate')
            });
            ...
            let useDefaultApiKey = this.settings.get_boolean('use-default-owm-key');
            ...
            currentRefreshSpinButton.connect('value-changed', (widget) => {
                this.settings.set_int('refresh-interval-current', 60 * widget.get_value());
            });
            forecastRefreshSpinButton.connect('value-changed', (widget) => {
                this.settings.set_int('refresh-interval-forecast', 60 * widget.get_value());
            });
            disableForecastSwitch.connect('notify::active', (widget) => {
                ...
                this.settings.set_boolean('disable-forecast', widget.get_active());
            });
            systemIconsSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('use-system-icons', widget.get_active());
            });
            startupDelaySpinButton.connect('value-changed', (widget) => {
                this.settings.set_int('delay-ext-init', widget.get_value());
            });
            temperatureUnitRow.connect('notify::selected', (widget) => {
                this.settings.set_enum('unit', widget.selected);
            });
            windSpeedUnitRow.connect('notify::selected', (widget) => {
                this.settings.set_enum('wind-speed-unit', widget.selected);
            });
            pressureUnitRow.connect('notify::selected', (widget) => {
                this.settings.set_enum('pressure-unit', widget.selected);
            });
            providerTranslateSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('owm-api-translate', widget.get_active());
            });
