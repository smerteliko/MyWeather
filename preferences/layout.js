...
var Layout = GObject.registerClass(
    class MyWeatherLayoutPage extends Adw.PreferencesPage {
        _init(settings) {
            super._init({
                title: _("Layout"),
            ...
            let panelPositionRow = new Adw.ComboRow({
                ...
                selected: this.settings.get_enum('position-in-panel')
            });
            ...
            let positionOffsetSpinButton = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({
                    ...
                    page_increment: 1,
                    page_size: 0,
                    value: this.settings.get_int('position-index')
                }),
                climb_rate: 1,
                digits: 0,
                numeric: true,
            ...
            let temperatureInPanelSwitch = new Gtk.Switch({
                ...
                active: this.settings.get_boolean('show-text-in-panel')
            });
            ...
            let conditionsInPanelSwitch = new Gtk.Switch({
                ...
                active: this.settings.get_boolean('show-comment-in-panel')
            });
            ...
            let weatherPopupPositionScale = new Gtk.Scale({
                adjustment: new Gtk.Adjustment({
                    ...
                    step_increment: 0.1,
                    page_increment: 2,
                    value: this.settings.get_double('menu-alignment')
                }),
                width_request: 200,
                show_fill_level: 1,
            ...
            let windArrowsSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('wind-direction')
            });
            ...
            this.providerTranslations = this.settings.get_boolean('owm-api-translate');
            let translateConditionsSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('translate-condition')
            });
            ...
            let temperatureDigitsRow = new Adw.ComboRow({
                ...
                selected: this.settings.get_int('decimal-places')
            });
            ...
            let locationLengthSpinButton = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({
                    ...
                    step_increment: 1,
                    page_increment: 10,
                    value: this.settings.get_int('location-text-length')
                }),
                climb_rate: 5,
                digits: 0,
                numeric: true,
            ...
            // Forecast Options
            this.disableForecast = this.settings.get_boolean('disable-forecast');
            ...
            let centerForecastSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('center-forecast')
            });
            ...
            let forecastConditionsSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('show-comment-in-forecast')
            });
            ...
            let forecastDaysRow = new Adw.ComboRow({
                ...
                model: forecastDays,
                selected: this.settings.get_int('days-forecast')
            });

            // Keep forecast expanded
            let forecastExpandedSwitch = new Gtk.Switch({
                valign: Gtk.Align.CENTER,
                active: this.settings.get_boolean('expand-forecast')
            });
            ...
            panelPositionRow.connect("notify::selected", (widget) => {
                this.settings.set_enum('position-in-panel', widget.selected);
            });
            positionOffsetSpinButton.connect('value-changed', (widget) => {
                this.settings.set_int('position-index', widget.get_value());
            });
            temperatureInPanelSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('show-text-in-panel', widget.get_active());
            });
            conditionsInPanelSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('show-comment-in-panel', widget.get_active());
            });
            weatherPopupPositionScale.connect('value-changed', (widget) => {
                this.settings.set_double('menu-alignment', widget.get_value());
            });
            windArrowsSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('wind-direction', widget.get_active());
            });
            translateConditionsSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('translate-condition', widget.get_active());
            });
            temperatureDigitsRow.connect("notify::selected", (widget) => {
                this.settings.set_int('decimal-places', widget.selected);
            });
            locationLengthSpinButton.connect('value-changed', (widget) => {
                this.settings.set_int('location-text-length', widget.get_value());
            });
            centerForecastSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('center-forecast', widget.get_active());
            });
            forecastConditionsSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('show-comment-in-forecast', widget.get_active());
            });
            forecastDaysRow.connect("notify::selected", (widget) => {
                this.settings.set_int('days-forecast', widget.selected);
            });
            forecastExpandedSwitch.connect('notify::active', (widget) => {
                this.settings.set_boolean('expand-forecast', widget.get_active());
            });
            // Detect settings changes to enable/disable related options
            this.settings.connect('changed', () => {
                if (this.disableForecastChanged()) {
                    if (this.settings.get_boolean('disable-forecast')) {
                        forecastGroup.set_sensitive(false);
                    } else {
                        forecastGroup.set_sensitive(true);
                    }
                }
                else if (this.providerTranslationsChanged()) {
                    if (this.settings.get_boolean('owm-api-translate')) {
